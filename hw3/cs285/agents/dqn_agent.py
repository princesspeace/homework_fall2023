...
class DQNAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.02) -> int:
        ...
        best_action = self.critic(observation).argmax(dim=1).item()
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_double_q:
                ...
                next_action = next_qa_values_online.argmax(dim=1, keepdim=True)
            else:
                next_action = next_qa_values.argmax(dim=1, keepdim=True)
                
            next_q_values = next_qa_values.gather(1, next_action)
            target_values = reward.unsqueeze(-1) + self.discount * next_q_values * (1-done.int().unsqueeze(-1))
