...
class ModelBasedAgent(nn.Module):
    ...
    def update(self, i: int, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        predicted_delta_obs = self.dynamics_models[i](norm_obs_acs)
        delta_obs = next_obs - obs
        norm_delta_obs = (delta_obs - self.obs_delta_mean) / (self.obs_delta_std + 1e-8)
        loss = self.loss_fn(predicted_delta_obs, norm_delta_obs)
    ...
    def update_statistics(self, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        self.obs_delta_std = delta_obs.std(dim=0)
    ...
    def get_dynamics_predictions(
        ...
        norm_obs_acs = (obs_acs - self.obs_acs_mean) / (self.obs_acs_std + 1e-8)
        predicted_delta_obs = self.dynamics_models[i](norm_obs_acs)
        unnorm_predicted_delta_obs = predicted_delta_obs * self.obs_delta_std + self.obs_delta_mean
        pred_next_obs = obs + unnorm_predicted_delta_obs
