...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        rewards = np.hstack(rewards).reshape(-1,1)
        terminals = np.hstack(terminals).reshape(-1,1)
        q_values = np.hstack(q_values).reshape(-1,1)
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            if self.gae_lambda is None:
                ...
                deltas = rewards + (1 - terminals) * self.gamma * values[1:] - values[:-1]
                ...
                for i in reversed(range(batch_size)):
                    ...
                    advantages[i] = deltas[i] + (1 - terminals[i]) * self.gamma * self.gae_lambda * advantages[i + 1]
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        total_reward = sum([(self.gamma ** t) * r for t, r in enumerate(rewards)])
